...
data = pd.read_excel(r'C:/Users/edwardt/SCCWRP/SMC BMP Projects - Regional monitoring network/EMC Calculator/Flow Calculations from Depth - M1 Backwater Curve/Flow_calculator_repo/FlowCalc_Template.xlsx', sheet_name='Caltrans 1194 - Dec Check')
...
def settings_SI():
    ...
    for ii in range(len(y_time)):
        ...
        y1_depth[ii] = y1_depth[ii] * cfs_to_cms + zz_drop_m
    ...
    fig, ax = plt.subplots()
    fig.suptitle('ASF Depth vs Time')
    ax.plot(y_time, y1_depth, color='Red', label='y1')
    ax.plot(y_time, y2_depth, color='Blue', label='y2')
    ...
    ax.set(xlabel="Time since start (s)", ylabel="Depth (cm)")
    ax.legend()
    plt.show()
...
Qin_energy = determine_hydrographs(y1_depth, y2_depth, y_time, Qex_flow, Qex_time, nMannings, [])
